    ...
    fetch(`/api/Sessions/${sessionId}`)
        ...
        .then(data => {
            ...
            if (allOptions.length > 0) {
                document.getElementById('results').innerHTML = `<h2>Results:</h2>${allOptions.map(result => `<p>${result.option}: ${result.count} votes</p>`).join('')}`;
            } else {
                document.getElementById('results').innerHTML = `<h2>Results:</h2>${scenario.options.map(option => `<p>${option}: 0 votes</p>`).join('')}`;
            }
        }).catch(error => {
            console.error('Error fetching session data:', error);
        });
...
window.onload = function() {
    ...
    document.getElementById('sessionCode').textContent = `Session Code: ${sessionId}`;
};
